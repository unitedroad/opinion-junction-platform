...
(function() {
  ...
  angular.module('testApp').directive('imageCrop', function() {
    return {
	...
	croppedDataUri: '='
	  ...
	  if (typeof imageCropId === 'undefined' || imageCropId == null) {
	      scope.imageCropId = 0;
	      imageCropId = 0;
	  } else {
	      scope.imageCropId = imageCropId + 1;
	...
	//scope.imageCropId = imageCropId;
	  ...
	  var $canvas = $('#canvas-' + scope.imageCropId)[0];
	  var ctx = $canvas.getContext('2d');
          files = [];
          zoom = 1;
	...
	scope.finalImgOnLoad = function() {
	  ...
	  //var scope = $($img).scope();
          ...
          scope.updateDragBounds();

	  
	  scope.finalImg = $("#final-cropped-image-" + scope.imageCropId)[0];
	  scope.finalImg.onload = scope.finalImgOnLoad;
        };
	  ...
	  var ctx = $canvas.getContext('2d');
          ctx.clearRect(0, 0, $canvas.width, $canvas.height);
          ctx.drawImage(scope.$img, x, y, newWidth, newHeight);
        }

        var to2Dp = function(val) {
          return Math.round(val * 1000) / 1000;
        }

        scope.updateDragBounds = function() {
          // $img.width, $canvas.width, zoom
          
          minXPos = $canvas.width - (scope.$img.width * zoom) - 50;
          minYPos = $canvas.height - (scope.$img.height * zoom) - 50;
	  ...
	  var ctx = $canvas.getContext('2d');
          ...
          if (proposedZoomLevel === maxZoomedInLevel) {
            ...
            ctx.clearRect(0, 0, $canvas.width, $canvas.height);
            ctx.drawImage(scope.$img, 0, 0, $canvas.width, $canvas.height);
            return;
          }

          newWidth = scope.$img.width * zoom;
          newHeight = scope.$img.height * zoom;
          ...
          ctx.clearRect(0, 0, $canvas.width, $canvas.height);
          ctx.drawImage(scope.$img, newXPos, newYPos, newWidth, newHeight);
        }
        ...
        scope.onCanvasMouseMove = function(e) {
          ...
          scope.moveImage(currentX - diffX, currentY - diffY);
